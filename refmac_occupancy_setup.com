...
awk '$1=="w"{$1=""} {print}' |\
awk 'NF==3{print "occupancy group id",++n,"chain",$2,"residue",$3,"alt",$1}\
     NF==2{print "occupancy group id",++n,"chain",$1,"residue",$2}' |\
awk 'BEGIN{print "occupancy refine"}\
     {print; res=$6" "$8} ! group[res]{++group[res];g[++n]=res}\
      {groups[res]=groups[res] " " $4;++count[g[n]]} END{\
   for(i=1;i<=n;++i){\
      inc="";if(count[g[i]]==1) inc="in";\
